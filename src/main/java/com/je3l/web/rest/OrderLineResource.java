...
/**
 * REST controller for managing {@link com.je3l.domain.OrderLine}.
 */
@RestController
...
public class OrderLineResource {
    ...
    public ResponseEntity<OrderLine> createOrderLine(@Valid @RequestBody OrderLine orderLine) throws URISyntaxException {
        ...
        orderLineSearchRepository.index(orderLine);
        ...
        Optional<OrderLine> result = orderLineRepository
            ...
            .map(existingOrderLine -> {
                ...
                if (orderLine.getPurchasePrice() != null) {
                    existingOrderLine.setPurchasePrice(orderLine.getPurchasePrice());
                }
            ...
            .map(savedOrderLine -> {
                orderLineSearchRepository.index(savedOrderLine);
    ...
    public ResponseEntity<Void> deleteOrderLine(@PathVariable("id") Long id) {
        ...
        orderLineSearchRepository.deleteFromIndexById(id);
    ...
    /**
     * {@code SEARCH  /order-lines/_search?query=:query} : search for the orderLine corresponding
     * to the query.
     *
     ...
     */
    @GetMapping("/_search")
    public List<OrderLine> searchOrderLines(@RequestParam("query") String query) {
        LOG.debug("REST request to search OrderLines for query {}", query);
        try {
            return StreamSupport.stream(orderLineSearchRepository.search(query).spliterator(), false).toList();
